...
library LibERC1155Diamond {
    ...
    function uri(uint256) internal view returns (string memory) {}
    ...
    function setApprovalForAll(address operator, bool approved) internal {
        ERC1155Storage.layout().isApprovedForAll[msg.sender][operator] = approved;
    ...
    function isApprovedForAll(address account, address operator) internal view returns(bool) {
        return ERC1155Storage.layout().isApprovedForAll[account][operator];
    }
    
    function totalSupply(uint256 id) internal view returns (uint256) {
        return ERC1155Storage.layout().totalSupply[id];
    }

    function exists(uint256 id) internal view returns (bool) {
        return ERC1155Storage.layout().totalSupply[id] > 0;
    }
    ...
    function safeTransferFrom(
        ...
        require(msg.sender == from || ERC1155Storage.layout().isApprovedForAll[from][msg.sender], "NOT_AUTHORIZED");

        ERC1155Storage.layout().balanceOf[from][id] -= amount;
        ERC1155Storage.layout().balanceOf[to][id] += amount;
        ...
        returns (uint256[] memory balances)
    {
        ...
        unchecked {
            for (uint256 i = 0; i < owners.length; ++i) {
                balances[i] = ERC1155Storage.layout().balanceOf[owners[i]][ids[i]];
            }
        }
    }
    ...
    function _mint(
        ...
        ERC1155Storage.layout().balanceOf[to][id] += amount;
        ERC1155Storage.layout().totalSupply[id] += amount;
    ...
    function _batchMint(
        ...
        for (uint256 i = 0; i < idsLength; ) {
            ERC1155Storage.layout().balanceOf[to][ids[i]] += amounts[i];
            ERC1155Storage.layout().totalSupply[ids[i]] += amounts[i];
    ...
    function _batchBurn(
        ...
        for (uint256 i = 0; i < idsLength; ) {
            ERC1155Storage.layout().balanceOf[from][ids[i]] -= amounts[i];
            ERC1155Storage.layout().totalSupply[ids[i]] -= amounts[i];
